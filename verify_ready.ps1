# ev_dynamics_simulation 依赖验证 - VS Insiders

Write-Host "================================" -ForegroundColor Cyan
Write-Host "依赖验证 - VS Insiders 版本" -ForegroundColor Cyan
Write-Host "================================" -ForegroundColor Cyan
Write-Host ""
...
if ((Test-Path $msvcPath) -and (Test-Path $clPath)) {
    Write-Host "  ✅ MSVC 14.50.35717" -ForegroundColor Green
    Write-Host "  ✅ cl.exe 编译器" -ForegroundColor Green
} else {
    Write-Host "  ❌ MSVC 或 cl.exe 未找到" -ForegroundColor Red
    $allPassed = $false
}
...
if ($allPassed) {
    ...
    Write-Host "`n立即开始编译:" -ForegroundColor Cyan
    Write-Host "  1. 配置 CMake PATH（如果未配置）" -ForegroundColor White
    Write-Host "     [Environment]::SetEnvironmentVariable('Path', `$env:Path + ';D:\Program Files\CMake\bin', 'Machine')" -ForegroundColor Gray
